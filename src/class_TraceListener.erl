...
% We cannot block the listener process on the os:cmd/1 call for LogMX, as this
% would prevent the listener to receive new trace messages. So we shall launch
% LogMX in the background. However then the closing of LogMX cannot be detected
% anymore, and the listener will not shutdown automatically once local
...
monitor( State ) ->
	...
	WaiterPid = spawn_link( fun() ->
		...
		case os:cmd( executable_utils:get_default_trace_viewer_path() ++ " "
					 ++ Filename ) of
				...
				Other ->
					error_logger:error_msg( "The trace listening failed: ~s.~n",
											[ Other ] )
